...
Patched version: improved parse_time_str_to_seconds to sanitize inputs coming from GitHub Actions
and handle surrounding quotes / invisible characters. Other logic unchanged from finalized script.
"""
...
def parse_time_str_to_seconds(s: str) -> int:
    """
    Parse many common time formats to seconds. Robust to surrounding quotes
    and invisible characters that sometimes come from CI inputs.

    Accepts: '2m47s', 2:47, 2.47, 167s, 167, "2m47s" (with quotes), and tolerates
    smart quotes and BOM/zero-width characters.
    """
    # Sanitize / normalize input first
    if s is None:
    ...
    # Remove surrounding quotes repeatedly (single, double, smart quotes)
    while len(s0) >= 2 and (
        (s0[0] == "'" and s0[-1] == "'")
        or (s0[0] == '"' and s0[-1] == '"')
        or (s0[0] == "\u2018" and s0[-1] == "\u2019")  # left/right single quotation mark
        or (s0[0] == "\u201c" and s0[-1] == "\u201d")  # left/right double quotation mark
    ):
        s0 = s0[1:-1].strip()

    # Remove common invisible/zero-width characters that break regex
    s0 = "".join(ch for ch in s0 if ord(ch) >= 32)

    s0 = s0.lower().strip()
    if not s0:
        raise ValueError("empty time string after sanitization")

    # Now parse formats: 1.23 (dot) meaning minutes.seconds, "m:s", "XmYs", plain seconds, integer
    ...
    if re.match(r'^\d+$', s0):
        return int(s0)
    raise ValueError(f"Could not parse time value '{s0}'")
