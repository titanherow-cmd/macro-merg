...
def add_time_of_day_fatigue(events, rng, is_exempted=False, max_pause_ms=0):
    if is_exempted:
        ...
        for i, e in enumerate(events):
            new_e = deepcopy(e)
            if i > 0:
                random_delay = rng.randint(0, max_pause_ms) if max_pause_ms > 0 else 0
                accumulated_delay += random_delay
            
            new_e['Time'] = int(e.get('Time', 0)) + accumulated_delay
            fatigued.append(new_e)
    ...
    try:
        ...
        for i, e in enumerate(events):
            ...
            if i > 0:
                ...
                curr_time = int(e.get('Time', 0))
                original_gap = curr_time - prev_time
                
                if original_gap > 0:
                    fatigued_gap = int(original_gap * fatigue_multiplier)
                    additional_delay = fatigued_gap - original_gap
                    accumulated_delay += additional_delay
            
            new_e['Time'] = int(e.get('Time', 0)) + accumulated_delay
            fatigued.append(new_e)
        
        return fatigued, fatigue_multiplier
        
    except Exception as ex:
        print(f"WARNING: Fatigue function failed: {ex}", file=sys.stderr)
        return events, 1.0
...
def generate_version_for_folder(files, rng, version_num, exclude_count, within_max_s, within_max_pauses, between_max_s, folder_path: Path, input_root: Path, selector, exemption_config: dict = None):
    ...
    for idx, fpath in enumerate(final_files):
        ...
        if not is_special:
            ...
            within_max_ms = int(within_max_s * 1000)
            
            if not is_desktop:
                zb_evs, _ = add_time_of_day_fatigue(zb_evs, rng, is_exempted=is_exempted, max_pause_ms=within_max_ms)
