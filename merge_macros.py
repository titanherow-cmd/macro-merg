#!/usr/bin/env python3
"""
merge_macros.py - INTEGRATED VERSION (Part 1 of 2)
Combines V6 anti-detection + V7 improvements + Handoff fixes
================================================================================
...
def write_counter(path: Path, n: int):
    try:
        ...
        print(f"Error writing counter: {e}", file=sys.stderr)
...
def process_macro_file(events: list[dict]) -> tuple[list[dict], int]:
    """
    ✅ HANDOFF FIX: Gentle event processing - only removes initial KeyUp/KeyDown
    """
    if not events:
        return [], 0
...
def preserve_click_integrity(events):
    """
    ✅ HANDOFF FIX: Protects ALL click types from time modification
    """
    preserved = []
...
def merge_events_with_pauses(base_events: list[dict], new_events: list[dict], pause_ms: int) -> list[dict]:
    """✅ V6 FEATURE: Robust event merging"""
    if not new_events:
        return base_events
...
def copy_always_files_unmodified(files, out_folder_for_group: Path):
    """✅ V6 FEATURE: Copy priority files unmodified"""
...
def add_desktop_mouse_paths(events, rng):
    """✅ V6 FEATURE: Mouse path interpolation"""
    if not events:
        return events
...
def add_reaction_variance(events, rng):
    """✅ HANDOFF FIX: Never delays protected events or MouseMoves before clicks"""
    varied = []
    prev_event_time = 0
    
    # First pass: Identify MouseMoves that precede clicks
    click_prep_moves = set()
    for i in range(len(events)):
        event_type = events[i].get('Type', '')
        is_click = any(t in event_type for t in ['Click', 'MouseDown', 'MouseUp', 
                                                   'LeftDown', 'LeftUp', 'RightDown', 'RightUp'])
        if is_click:
    ...
    for i, e in enumerate(events):
        ...
        if is_protected_event(e) or i in click_prep_moves:
...
def add_mouse_jitter(events, rng, is_desktop=False, target_zones=None, excluded_zones=None):
    """✅ V6 FEATURE: Coordinate randomization"""
...
def insert_intra_pauses(events, rng, is_time_sensitive=False, max_pause_s=33, max_num_pauses=3):
    """✅ HANDOFF FIX: Line 430 - Fixed walrus operator"""
    if not events:
        return deepcopy(events), []
    
    n = len(events)  # ✅ FIXED: Separate assignment
    if n < 2 or not is_time_sensitive:  # ✅ FIXED: No walrus operator
        return deepcopy(events), []
...
def add_afk_pause(events, rng):
    """✅ V6 FEATURE: AFK pause insertion"""
    if not events:
        return deepcopy(events)
    evs = deepcopy(events)
    if rng.random() < 0.7:
...
# END OF PART 1 - Continue with PART 2# PART 2 of 2 - Paste this AFTER PART 1
...
class NonRepeatingSelector:
    ...
    def select_unique_files(self, files, target_minutes):
        """✅ HANDOFF FIX: Removed max_files - uses as many files as needed"""
        if not files:
            return []
        ...
        # ✅ HANDOFF FIX: Keep adding files until target reached, NO FILE LIMIT
        while estimated_total < target_minutes:
            ...
            estimated_total += file_duration + AVG_INTER_FILE_PAUSE_MIN
            available.remove(chosen)
            self.used_files.add(chosen)
            
            if estimated_total >= (target_minutes * 0.95):
                break
        
        print(f"  Selected {len(selected)} files, estimated: {estimated_total:.1f}m (target: {target_minutes}m)")
        return selected
...
def generate_version_for_folder(files, rng, version_num, exclude_count, within_max_s, 
                                within_max_pauses, between_max_s, folder_path: Path, 
                                input_root: Path, selector, exemption_config: dict = None, 
                                target_minutes=35, folder_number=1):
    """✅ INTEGRATED: V6 logic + V7 folder numbering + 35min default"""
    if not files:
    ...
    exemption_config = exemption_config or {
    ...
    for idx, fpath in enumerate(final_files):
        ...
        if not is_special:
            ...
            # ✅ HANDOFF FIX: Only calculate duration, don't re-process
            file_duration_ms = zb_evs[-1]["Time"] if zb_evs else 0
            
            if is_time_sensitive:
                if not exemption_config.get("disable_intra_pauses", False):
                    intra_evs, _ = insert_intra_pauses(zb_evs, rng, is_time_sensitive=True, 
                                                       max_pause_s=within_max_s, max_num_pauses=within_max_pauses)
                else:
                    intra_evs = zb_evs
            else:
                intra_evs = zb_evs
    ...
    print(f"  → Generated: {total_minutes}m total (target: {target_minutes}m)")
    ...
    # ✅ V7 IMPROVEMENT: Add folder number (A1, B1, C1)
    parts_str = ' - '.join(parts)
    base_name = f"{letters}{folder_number}_{total_minutes}m= {parts_str}"
    
    MAX_FILENAME_LENGTH = 200
    if len(base_name) > MAX_FILENAME_LENGTH:
        file_count = len(final_files)
        base_name = f"{letters}{folder_number}_{total_minutes}m_{file_count}files"
...
def main():
    ...
    for folder in folder_dirs:
        folder_number = folder_numbers[folder]
        ...
        print(f"\nProcessing folder #{folder_number}: {rel_folder} ({len(files)} files available)")
