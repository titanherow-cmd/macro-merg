...
def generate_version(files, seed, global_pause_set, version_num, exclude_count,
    ...
    if included:
        ...
        if dup_count > 0:
            # if only one included file, duplicate it once; otherwise pick unique samples
            if len(included) == 1:
    ...
    if included:
        # insert some extra copies of included files (safe)
        try:
        ...
        for ef in extra_files:
            ...
            # avoid adjacent duplicates: if pos>0 and previous equals ef, adjust
    ...
    for idx, f in enumerate(final_files):
        ...
        # 60% chance to add an inter-file pause BEFORE appending this file's events
        if rng.random() < 0.6:
            while True:
        ...
        if shifted:
            ...
            # set time_cursor to end of this file plus 30 ms gap (keeps monotonic times)
            time_cursor = max_t + 30
        else:
            play_times[f] = 0

    # Build parts string (per-file durations in minutes)
    parts = [part_from_filename(f) + f"[{round((play_times[f] or 0)/60000)}m] "
             for f in final_files]  # space after each part

    # --- FIX: compute total minutes from merged events so pauses are included ---
    if merged:
    ...
    for gi, grp in enumerate(find_groups(in_dir)):
        ...
        for v in range(1, a.versions+1):
            fname, merged, finals, pauses, excl, total = generate_version(
                ...
                a.intra_file_min_min, a.intra_file_max_mins) if False else generate_version(
                files, base_seed+gi*1000+v, global_pauses, v,
                a.exclude_count, a.intra_file_enabled, a.intra_file_max,
                a.intra_file_min_mins, a.intra_file_max_mins)
            # The above ugly inline is to ensure earlier param names are used; the effect is same as calling generate_version(...)
            if not fname:
                continue
