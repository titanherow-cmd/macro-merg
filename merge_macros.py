#!/usr/bin/env python3
"""
merge_macros.py (fixed)
Merged and cleaned-up from the uploaded version to remove indentation / duplication problems.

Primary behavior:
- Walk `input_dir` for folders containing JSON files
- For each folder, generate `versions` merged outputs using non-repeating selection/shuffle logic
- Apply timing and anti-detection transforms (micro-pauses, jitter, fatigue, etc.)
- Write merged JSONs under output_dir/<merged_bundle_x>/...
- Package all written outputs into a zip

Notes:
- This file intentionally preserves the structure and helper functions from your uploaded version.
- I fixed duplicated lines and normalized the generate_version_for_folder signature and its call.
"""
...
# --------- simple anti-detection helpers (kept from uploaded file) ---------
...
# --------- generate for a single folder (fixed signature) ---------
def generate_version_for_folder(files, rng, version_num,
                                exclude_count,
                                within_max_s, within_max_pauses,
                                between_max_s,
                                folder_path: Path,
                                input_root: Path,
    ...
    if is_mobile_group:
        ...
        if special_cand:
            ...
            print(f"INFO: mobile group {folder_path} - special '{SPECIAL_FILENAME}' not found; skipping insertion.")
    merged = []
    ...
    for idx, fpath in enumerate(final_files):
        ...
        if not is_special:
            # Keep mobile-optimized defaults (as in your uploaded file)
            zb_evs, _ = add_time_of_day_fatigue_safe(zb_evs, rng)
        ...
        if is_special:
            ...
            # within_min_s default 0, within_max_s passed in seconds
...
# small safe wrapper for the time-of-day fatigue function: uploaded file provided a richer version.
def add_time_of_day_fatigue_safe(events, rng):
    try:
        ...
        if is_weekend:
            fatigue_min += 0.05; fatigue_max += 0.1
