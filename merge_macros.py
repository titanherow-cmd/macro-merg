...
Merged, patched version:
 - Recursively finds JSON files under each group (direct children of --input-dir).
 - Accepts extra CLI args: --within-max-time, --within-max-pauses, --between-max-time, --between-max-pauses.
 - Parses human-friendly times (e.g. 1m32s, 92s, 1:32).
 - Writes merged JSONs into --output-dir/<group>/ and creates merged_bundle_{N}.zip in --output-dir.
 - Maintains a persistent counter at .github/merge_bundle_counter.txt (script updates file)
   The workflow commits the counter file back to the repo so it persists across runs.
"""
...
def parse_time_to_ms(s: str) -> int:
    ...
    Accepts variants like '1m32s', '92s', '1:32', '1m', '45', '1.5m'.
...
def find_json_files_recursive(group_path: Path, output_dir: Path = None):
    ...
    for p in sorted(group_path.rglob("*.json")):
        # avoid picking up outputs placed under the output_dir (if output_dir is inside input tree)
...
def generate_version(files, seed, global_pause_set, version_num, exclude_count,
    ...
    for idx, f in enumerate(final_files):
        ...
        if idx < len(final_files) - 1:
            eff_min = between_min_ms if between_min_ms >= 0 else 0
            eff_max = max(eff_min, between_max_ms)
            if eff_max == 0:
                inter_ms = rng.randint(120000, 780000) if idx > 0 else rng.randint(120000, 180000)
            else:
                inter_ms = rng.randint(eff_min, eff_max)
            # ensure uniqueness optionally
...
def main():
    ...
    prev_processed = set()
    # read prev processed zip content if exists in out_dir
    try:
    ...
    # increment persistent counter and write zip
    counter = read_counter() + 1
    write_counter(counter)

    zip_name = f"merged_bundle_{counter}.zip"
